...
function _enRenderChecklist() {
    ...
    const tick = ok => ok
        ...
        : `<span style="color:var(--border-color)"><i class="ph ph-circle"></i></span>`;
...
function enAvancarFase(id) {
    ...
    p.status = nova;
    alert(`Projeto ${p.id} agora está em: ${nova}`);
    initEngenharia();
}
...
// ====== ARQUIVO SELECIONADO ======
function enAtualizarArquivo(input) {
    const label = document.getElementById('engFileLabel');
...
function enConfimarUploadDocPag() {
    ...
    const fileInput = document.getElementById('fileInputEng');
    const p = mockProjetos.find(x => x.id === projeto);
    const nomeArquivo = (fileInput && fileInput.files && fileInput.files.length > 0)
    ...
    alert(`Upload realizado!\nArquivo   : ${nomeArquivo}\nDocumento : ${tipo}\nProjeto   : ${p?.cliente || projeto}\nProx. passo: ${proximo[tipo] || '—'}`);
    // Reset file input
    if (fileInput) fileInput.value = '';
    const label = document.getElementById('engFileLabel');
    if (label) { label.textContent = 'PDF, JPG, PNG — máx. 20MB'; label.style.color = ''; }
}
...
function enRelatorioMensal() {
    const base = mockProjetos.filter(p => ['Projeto','Instalação','Homologação','Concluído'].includes(p.status));
    let r = `RELATORIO TECNICO MENSAL\n${new Date().toLocaleDateString('pt-BR')}\n${'='.repeat(40)}\n\n`;
    ['Projeto','Instalação','Homologação','Concluído'].forEach(f => {
        const g = base.filter(p=>p.status===f);
        r += `${f}: ${g.length} obras\n`;
    });
    r += `\nkWp concluido: ${base.filter(p=>p.status==='Concluído').reduce((s,p)=>s+p.potencia_kwp,0).toFixed(1)} kWp\n\n`;
    ...
    a.href     = url;
    a.download = `Relatorio-Tecnico-${new Date().toISOString().slice(0,10)}.txt`;
    a.click();
